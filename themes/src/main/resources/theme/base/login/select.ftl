            ...
            <select id="authenticators-choice" size="1">
                <#list authenticationSelections as authenticationSelection>
                    <#if authenticationSelection.credentialId?has_content>
                        <option value="${authenticationSelection.id}" <#if selectedCredential?has_content && authenticationSelection.credentialId == selectedCredential>selected</#if>>${msg('${authenticationSelection.authExecName}')}<#if authenticationSelection.showCredentialName()> - <#if authenticationSelection.credentialName?has_content>${authenticationSelection.credentialName}<#else>${authenticationSelection.credentialId}</#if></#if></option>
                    <#else >
