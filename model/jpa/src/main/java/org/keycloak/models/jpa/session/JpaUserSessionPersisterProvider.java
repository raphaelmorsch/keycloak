    ...
    public void removeAllExpired() {
        ...
        if (KeycloakModelUtils.isRealmProviderJpa(session)) {
            ...
            for (int offlineSessionIdleTimeout : offlineSessionIdleTimeouts) {
                ...
                logger.infof("Removed %d expired user sessions and %d expired client sessions for the offlineSessionIdleTimeout %d", us, cs, offlineSessionIdleTimeout);

                numSessionsDeleted += us;
                numClientSessionsDeleted += cs;
            }
            // TODO:mposolda debug
            logger.infof("Removed %d expired user sessions and %d expired client sessions in all realms", numSessionsDeleted, numClientSessionsDeleted);
        } else {
