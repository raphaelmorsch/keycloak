    ...
    private static List<ProviderConfigProperty> getProps(ComponentModel parent) {
        ...
        return ProviderConfigurationBuilder.create()
                    ...
                    .property().name(GroupMapperConfig.MODE)
                               .label("Mode")
                               .helpText("LDAP_ONLY means that all group mappings of users are retrieved from LDAP and saved into LDAP. UNSYNCED is Read-only LDAP mode where group mappings are " +
                            ...
                            "they are saved to local keycloak DB. " +
                               "READ_ONLY is Read-only LDAP mode where group mappings are retrieved from LDAP and it is not allowed to add/remove any group mappings for user.")
                               .type(ProviderConfigProperty.LIST_TYPE)
                               .options(MODES)
                               .defaultValue(mode)
                               .add()
