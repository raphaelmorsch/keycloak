...
public class OAuth2CodeParser {
     ...
     * @param clientSession
     * @param codeData
     * @return Instance with all dat
     */
    public static ParseResult persistCode(KeycloakSession session, AuthenticatedClientSessionModel clientSession, OAuth2Code codeData) {
        ...
        codeStore.put(key, serialized);

        String code = key.toString() + "." + clientSession.getUserSession().getId() + "." + clientSession.getClient().getId();

        return new ParseResult(code, codeData, clientSession);
    }
    ...
    public static ParseResult parseCode(KeycloakSession session, String code, RealmModel realm, EventBuilder event) {
        ...
        if (codeData == null) {
            logger.warnf("Code '%s' already used for userSession '%s' and client '%s'.", userSessionId, result.clientSession.getClient().getClientId());
            return result.illegalCode();
        }

        if (logger.isTraceEnabled()) {
            logger.tracef("Successfully verified code '%s'. User session: %s, client: %s", code, userSessionId, result.clientSession.getClient().getClientId());
        }
